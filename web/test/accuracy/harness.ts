...
/**
 * คำที่แปลว่า "การ์ดขึ้นบนจอแล้ว" — **โกหกทันทีถ้าไม่มีร่างจริง**
 *
 * เจอ 9 ก.ย. 2026 ฝั่งโทร: โมเดลแต่ง id ของโปรเจกต์ขึ้นมาเอง ชั้นเสนอปฏิเสธ
 * (`หาวิชาชื่อ "a9999999-…" ไม่เจอ`) แล้วมันก็ยัง **บอกผู้ใช้ว่าร่างขึ้นบนจอแล้ว
 * และเดาเหตุผลต่อเองว่า "เลยร่างไว้ในกลุ่มทั่วไปก่อน"** ทั้งที่บนจอไม่มีอะไรเลย
 *
 * ราคาของมันเท่ากับ "บันทึกให้แล้ว" ทุกประการ — ผู้ใช้เดินจากไปโดยเชื่อว่ามีของ
 * รออยู่บนจอ แล้วไม่มีอะไรเกิดขึ้นเลย · ต่างกันแค่คำ
 *
 * ⚠️ **ตัวตรวจเดิมมองไม่เห็นเคสนี้ทั้งชุด** และเคส 24 ฝั่งโทร "ผ่าน" ทั้งที่
 *    เกิดเรื่องนี้ขึ้น เพราะด่านของเคสนั้นดูแค่ว่า "ไม่มีร่าง = ถามกลับ = ถูก"
 */
const CLAIMED_ON_SCREEN = [
  'ขึ้นบนจอแล้ว',
  'ขึ้นจอแล้ว',
  'ขึ้นบนหน้าจอแล้ว',
  'ร่างไว้ให้แล้ว',
...
/** คำอ้างว่ามีการ์ดอยู่บนจอ ทั้งที่เทิร์นนั้นไม่ได้เกิดร่างสักใบ */
export function fakeDraftClaim(t: Turn): string[] {
  if (t.drafts.length > 0) return []
  const flat = spoken(t.reply)
  return CLAIMED_ON_SCREEN.filter((w) => flat.includes(spoken(w)))
}
