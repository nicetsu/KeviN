...
export const CASES: Case[] = [
   ...
   *
   * สองข้อนี้วัดคนละครึ่งของกติกาข้อ 13 · ข้อ 18 วัดว่า "อ่านออกแล้วทำอะไรต่อ"
  ...
  /*
   * 25–26 · ย้าย/แก้โปรเจกต์ (10 ก.ย. 2026)
   *
   * ก่อนหน้านี้โปรเจกต์ที่สร้างแล้วแก้อะไรไม่ได้เลย — ทางเดียวคือเก็บเข้าคลัง
   * แล้วสร้างใหม่ ซึ่งพางานข้างในไปด้วย · ความเสี่ยงใหม่ของรอบนี้คือ
   * **มันไปสร้างใบใหม่แทนที่จะแก้ใบเดิม** ซึ่งจบลงด้วยโปรเจกต์ซ้ำสองใบ
   * ที่ลบทิ้งจากประตูนี้ไม่ได้ · เคส 25 จึงตรวจ "ห้ามมี add_project" ด้วย
   */
  {
    id: '25',
    title: 'ย้ายโปรเจกต์ข้ามกลุ่ม — ต้องแก้ใบเดิม ไม่ใช่สร้างใหม่',
    say: 'ย้ายโปรเจกต์เรื่องส่วนตัวไปอยู่กลุ่มฝึกงานหน่อย',
    want: 'propose_edit_project · projectId เดิม · ย้ายไปกลุ่ม "ฝึกงาน"',
    check: (t) => {
      const out: string[] = []
      if (called(t, 'propose_add_project')) {
        out.push('สร้างโปรเจกต์ใบใหม่แทนที่จะย้ายใบเดิม — ของซ้ำลบจากตรงนี้ไม่ได้')
      }
      const bad = one(t, 'edit_project')
      if (bad.length) return [...out, ...bad]
      const a = acted(t)
      if (a?.kind !== 'edit_project') return [...out, ...bad]
      if (a.projectId !== P.life) out.push('แก้ผิดโปรเจกต์')
      if (a.areaId !== 'c5') out.push(`ย้ายไปกลุ่ม ${a.areaId} ควรเป็น ฝึกงาน (c5)`)
      return out
    },
  },
  {
    id: '26',
    title: 'เปลี่ยนชื่อโปรเจกต์ — ห้ามแตะกลุ่มที่ไม่ได้สั่งให้ย้าย',
    say: 'เปลี่ยนชื่อโปรเจกต์เรื่องส่วนตัวเป็นเรื่องส่วนตัวและการเงิน',
    want: 'propose_edit_project · เปลี่ยนแค่ชื่อ · areaId ต้องไม่ถูกใส่มา',
    check: (t) => {
      const bad = one(t, 'edit_project')
      if (bad.length) return bad
      const a = acted(t)
      if (a?.kind !== 'edit_project') return bad
      const out: string[] = []
      if (a.projectId !== P.life) out.push('แก้ผิดโปรเจกต์')
      if (!a.name?.includes('การเงิน')) out.push(`ชื่อใหม่เป็น "${a.name}"`)
      /*
       * ⚠️ **ฟิลด์ที่ไม่ได้สั่งให้แก้ ต้องไม่เดินทางมาถึงชั้นเขียน** — ส่ง `area`
       *    มาด้วยความหวังดีจะทำให้โปรเจกต์ย้ายกลุ่มโดยที่ผู้ใช้ไม่ได้ขอ และ
       *    การ์ดจะโชว์บรรทัด "กลุ่ม" ที่เขาไม่ได้พูดถึงเลย
       */
      if (a.areaId !== undefined) out.push('ใส่ areaId มาทั้งที่ไม่ได้สั่งให้ย้ายกลุ่ม')
      return out
    },
  },
]
 ...
 *    และ prompt ฝั่งโทรก็ไม่มีกติกาข้อ 13 อยู่ด้วยซ้ำ (lib/ai/prompt.ts)
 */
export const VOICE_IDS = ['01', '04', '05', '09', '11', '13', '16', '20', '21', '22', '24', '25']
