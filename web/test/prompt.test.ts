...
test('ประโยคปิดท้ายอยู่ **ฝั่งแชตเท่านั้น** — ฝั่งโทรต้องไม่ถูกแตะ', () => {
  /*
   * ปัญหา "เรียบร้อยแล้ว" วัดเจอที่**ฝั่งแชตที่เดียว** (8 ก.ย. 2026)
   * — แก้ตรงที่วัดเจอ แล้ว prompt ฝั่งโทรจะเท่ากับของเดิมทุกตัวอักษร
   * ซึ่งแปลว่าการแก้รอบนี้ทำให้ฝั่งโทรถอยหลังไม่ได้เลยโดยไม่ต้องเดา
   *
   * เทสต์นี้ตรึงคุณสมบัตินั้นไว้ · ถ้าวันไหนย้ายเข้า `CORE` อีก
   * ต้องรันชุดวัด **ฝั่งโทร** ก่อนและหลังด้วย ไม่ใช่แค่ฝั่งแชต
   */
  assert.equal(systemPrompt('voice', TODAY).includes('ทานแล้วกดยืนยันได้เลย'), false)
})
