...
export async function runTool(
  ...
  try {
    ...
    //
    // กติกาที่เคยอ้างถึงมันใน `lib/ai/prompt.ts` **ถูกลบแล้ว 9 ก.ย. 2026** ตอนแตะ
    // prompt รอบที่ต้องรันชุดวัดอยู่แล้ว — ไม่ต้องจ่ายโทเคนให้กฎที่เป็นจริงไม่ได้อีก
    //
    // ที่ยังคง **ฟิลด์** ไว้เพราะ `tool_calls.rows_hidden` เก็บค่านี้ และรูปของ
    // `ToolResult` ถูกอ่านอยู่หลายที่ · ถอดฟิลด์เป็นงานคนละก้อนกับการเก็บกฎที่ตายแล้ว
